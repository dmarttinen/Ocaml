...
struct
  ...
  *)
  ...
  match map with
    ...
    |Node(node) ->                                  (*otherwise*)
      let diff = KVMod.compare_keys key node.key in     (*use compare to find if the current key is greater than less than or equal to the given key*)
  ...
  *)
  let contains_key map key =
    getopt map key != None    (*check if a value is returned from getopt*)
  ;;
  ...
  *)
  let to_string map =
    ...
    let build buf k v =                                                           (*helper function that will be called by fold*)
      let kvstr = Printf.sprintf "%s, " (KVMod.keyval_string k v) in                            (*use sprintf to convert the k,v pair intto a formatted string*)
  ...
  *)
  let rec remove_key map key =
    ...
    | Node(node) ->
      let diff = KVMod.compare_keys key node.key in                                                                         (*use compare to check if the current key is equal to, less than or greater than the given key*)
