...
(*parse a boolean or expression, this is left associative*)
and parse_or toks =
...
(*parse a boolean and expression, this is left associative*)
and parse_and toks =
  let rec iter lexpr toks =
    match toks with
    | AndTok :: rest ->
...
and parse_cond toks =
  match toks with
  | If :: rest ->                                                   (*check for the If token*)
    begin
      let (if_expr,rest) = parse_expr rest in                       (*grab the test expression*)
      match rest with
      | Then :: rest ->                                             (*check for the Then token*)
        begin
          let (then_expr,rest) = parse_expr rest in                 (*grab the then expression*)
          match rest with
          | Else :: rest ->                                         (*check for the Else token*)
            let (else_expr,rest) = parse_expr rest in               (*grab the else expression*)
            (Cond{if_expr; then_expr; else_expr}, rest)             (*return the Cond with the three epxressions*)
          | _ ->                                                    (*if the Else token is missing, throw an error*)
              raise (ParseError{msg="syntax error"; toks=rest});
        end
      | _ ->                                                        (*if the Then token is missing, throw an error*)
          raise (ParseError{msg="syntax error"; toks=rest});
    end
  | _ -> parse_lambda toks
...
and parse_lambda toks =
  match toks with
  | At :: Ident param_name :: rest -> let (code_expr,rest) = parse_expr rest in                   (*match with an At and then an Ident*)
    (Lambda{param_name; code_expr}, rest)                                                         (*return thr Lambda*)
  | At :: _ ->  raise (ParseError{msg="Expected identifier in lambda expression after @"; toks})  (*if not an Ident after the At then error*)
  | _ -> parse_apply toks
