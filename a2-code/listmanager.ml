...
let execute_command tokens =
  match tokens with
    | [||] -> ()
    | [|cmd|] when cmd = "help" -> Printf.printf "%s\n" help_string
    | [|cmd|] when cmd = "quit" -> quit_now := true
    | [|cmd|] when cmd = "show" -> begin
                                    Printf.printf "--BEG LIST--\n";
                                    Sortedlist.print !Undolist.curr_list;
                                    Printf.printf "--END LIST--\n"
                                   end
    | [|cmd|] when cmd = "clear" -> Undolist.set_to_list []
    | [|cmd;rest|] when cmd = "add" -> Undolist.add_elem tokens.(1)
    | [|cmd;rest|] when cmd = "remove" -> Undolist.remove_elem tokens.(1)
    | [|cmd;rest|] when cmd = "save" -> Util.strlist_to_file !Undolist.curr_list tokens.(1)
    | [|cmd;rest|] when cmd = "load" -> Undolist.set_to_list (Util.strlist_from_file tokens.(1))
    | [|cmd;rest|] when cmd = "mergein" -> Undolist.set_to_list (Sortedlist.merge (Util.strlist_from_file tokens.(1)) (!Undolist.curr_list))
    | [|cmd|] when cmd = "undo" -> if Undolist.undo() = false then
                                      Printf.printf "WARNING: undo list empty, no changes made\n"
                                   else
                                      ()
    | [|cmd|] when cmd = "redo" -> if Undolist.redo() = false then
